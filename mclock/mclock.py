...
import math
import itertools
import learnTkinterimport threading
...
def gen_i_pos(c_pntr,long_pntr):
    for i,p in enumerate((get_clock)step(c_pntr,long_pntr)):
        yield i,p

class Pointer:
    def __init__(self,c_cptr,long_pntr,cvns,scale=None,super_pntr=None,width=1,file='blak'):
        # 参数说明
        # c_pntr:表盘的中心超标；
        # long_pntr: 表针长；
        # cvns：画布引用；
    ...
    def walk_step(selfself):
        self.delete()
        self.draw()
        i,self.super_pntr = self.pos_iter.__next__()
...
class PlateImg:
    ...
    def draw(self):
        ...
        self.pid = self.cvns.create_image(self.c_pntr,img = self.im)
...
class CustomPlae:
    def __init__(self,c_pntr,clock_r,cvns,img='platex.gif',imgi='flowersx.gif'):
        self.pi = PlateImg(c_pntr,clock_r,cvns,imgp)
        self.ii = InImg(c_pntr,clock_r,cvns,imgi)

    def set_img(selfself,imgp,imgi):
        self.pi.delete()
        self.ii.delete()
...
class MyClock:
    ...
    def __init__(self,base_pntr,clock_r,cvns):
        ...
        self.m_pntr = Pointer(base_pntr,4 * clock_r // 5,self.cvns,12,super_pntr=self.pntr)
        #上一行再核对一下
        self.h_pntr.set_start(h * 5)
        self.m_pntr.set_start(m)
        self.h_pntr.walk_step()
        self.m_pntr.walk_step()
        self.s_pntr =Pointer(base_pntr,clock_r-5,self.cvns,60,super_pntr=self.m_pntr,fill=)
        #检查上一行
    ...
    def chg_plate(self):
        ...
        self.m_pntr.draw()
    def set_img(selfself,imgp,imgi):
        if not isinstance(self.plate,CustomPlae):
            self.chg_plate()
...
class MyButton:
    def __init__(self,root,clock):
        self.clock = clock
        buttpn = tkinter.Button(root,text = '改变表盘', command = self.chg_plate)
        button.pack()
